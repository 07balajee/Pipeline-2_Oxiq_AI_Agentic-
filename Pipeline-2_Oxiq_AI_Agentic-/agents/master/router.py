...
class Router:
    ...
    def route(self, current_state: str, event_name: str) -> Tuple[str, Optional[str]]:
        ...
        # 5. Move to HR interview pending -> dispatch Agent 8 Turn 1 (HR evaluation form)
        ...
        # 6. Turn 1 handed HR a form -> pause for HR to submit ratings (HREvaluationApproval gate)
        elif current_state == STATE_HR_INTERVIEW_PENDING and event_name == EVENT_HR_EVALUATION_REQUESTED:
            return STATE_HR_EVALUATION_AWAITED, None

        # 6b. HR submitted ratings -> dispatch Agent 8 Turn 2 (compute HR score + rank cohort)
        elif current_state == STATE_HR_EVALUATION_AWAITED and event_name == EVENT_HR_EVALUATION_SUBMITTED:
            return STATE_HR_RANKING_PENDING, AGENT_HR_RANKING

        # 6c. Turn 2 produced a ranking -> pause for hiring manager review (HRRankingApproval gate)
        elif current_state == STATE_HR_RANKING_PENDING and event_name == EVENT_HR_SCORE_SUBMITTED:
            return STATE_CANDIDATE_RANKING_AWAITED, None

        # 6d. Ranking approved -> dispatch Agent 8 Turn 3 (persist outcome)
        elif current_state == STATE_CANDIDATE_RANKING_AWAITED and event_name == EVENT_CANDIDATE_RANKING_APPROVED:
            return STATE_HR_INTERVIEW_COMPLETED, AGENT_HR_RANKING
